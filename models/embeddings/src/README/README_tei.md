...
```bash
your_port=8090
curl http://localhost:$your_port/v1/embeddings \
-X POST \
...
2. Run the embedding microservice connected to TEI service:
   ...
   ```bash
   export TEI_EMBEDDING_ENDPOINT=http://<tei_server_host>:<tei_server_port>
   docker run --rm -d --name="embedding-tei-server" \
   -p 6000:6000 \
...
2. Navigate to the Docker Compose directory:
   ...
   ```bash
   cd AIComps/models/embeddings/deployment/docker_compose/
   ```
