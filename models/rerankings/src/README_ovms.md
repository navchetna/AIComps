...
It is Intel's highly optimized serving solution which employs OpenVINO Runtime for super fast inference on CPU.
...
It speedup starting the service and avoids copying the model from Internet each time the container starts.
   ...
   cd AIComps/models/rerankings
   uv venv --python=3.12 && source .venv/bin/activate

   # Install deps
   uv pip install -r https://raw.githubusercontent.com/openvinotoolkit/model_server/refs/heads/releases/2025/0/demos/common/export_models/requirements.txt

   # Install compatible version of torch
   uv pip install torch==2.8.0 --extra-index-url "https://download.pytorch.org/whl/cpu"

   # Download the script that converts model into OV-IR format
...
- For Xeon CPU:
   ...
   export rest_port=8002
   docker run --rm -d -p $grpc_port:8001 -p $rest_port:8002 -v ./models:/models --name ovms-rerank-serving \
   openvino/model_server:2025.0 --port $grpc_port --rest_port $rest_port --rest_workers 16 --config_path models/config_reranking.json
   ```
...
1. Build the Docker image for the reranking microservice:
   ...
   ```bash
   cd ../../ # to the AIComps dir
   docker build -t navchetna/reranking:comps --build-arg SERVICE=ovms -f models/rerankings/src/Dockerfile .
   ```

2. Run the reranking microservice and connect it to the OVMS service:

   ```bash
    export OVMS_RERANKING_PORT=8002 # Use HTTP port
    export OVMS_RERANKING_ENDPOINT="http://<ovms_server_host>:${OVMS_RERANKING_PORT}"
    export MODEL_ID=BAAI/bge-reranker-large

   docker run -d --rm --name="reranking-ovms-server" -e LOGFLAG=True  -p 8008:8000 --ipc=host -e OVMS_RERANKING_ENDPOINT=$OVMS_RERANKING_ENDPOINT -e RERANK_COMPONENT_NAME="OPEA_OVMS_RERANKING" -e MODEL_ID=$MODEL_ID navchetna/reranking:comps
   ```
...
- Verify the reranking service is running:
  ...
  ```bash
    curl http://localhost:8008/v1/health_check \
    -X GET \
    -H 'Content-Type: application/json'
  ```
...
- Execute reranking process by providing query and documents
  ...
  ```bash
  curl http://localhost:8008/v1/reranking -X POST -H 'Content-Type: application/json' \
  ...
  ```bash
  curl http://localhost:8008/v1/reranking \
    -X POST \
...
2. Model Selection:
   ...
   It should be exported to the models repository and set in 'MODEL_ID' env in the deployment of the API wrapper.
